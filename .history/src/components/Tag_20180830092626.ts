...
export default abstract class Tag<P extends typeof props, S extends state> extends JqueryTag<P, S> {
    ...
    componentWillReceiveProps(nextProps: P) {
        ...
        // let keys = [];
        let keys = ["onChange", "onBlur", "style", "onClick", "onFocus", "onKeyDown", "onKeyPress", "onKeyUp", "onMouseDown", "onMouseMove", "onMouseOut", "onMouseOver", "onMouseUp", "onPressEnter", "category", "value"];
        keys.forEach(key => {
            delete state[key];
        });
        this.setState(state);
