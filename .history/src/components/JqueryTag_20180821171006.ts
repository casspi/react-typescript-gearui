...
export default class JqueryTag<P extends typeof props, S extends state> extends React.Component<P, S> {
        ...
        for(let key in props) {
            ...
            if(event instanceof Function) {
                ...
                if(keyLower.startsWith("on")) {
                    this.bind(keyLower.substring(2, key.length), event);
                }else {
                ...
                if(event instanceof Array) {
                    event.forEach(element => {
                        let keyLower = key.toLowerCase();
                        if(keyLower.startsWith("on")) {
                            this.bind(keyLower.substring(2, key.length), element);
                        }else {
    ...
    protected addGetterAndSetterInState(state: S) {
        let className = this.constructor.name;
        let propsTemplete = G.tag[className].props;
        if(propsTemplete) {
            for(let key in state) {
                let vInState = state[key];
                if(vInState instanceof Function && propsTemplete[key] != Constants.TYPE.Function) {
                    let v = vInState();
                    delete state[key];
                    Object.defineProperty(state, key, {
                        get: () => {
                            if(propsTemplete[key]) {
    ...
    // 将字符串转为整数
    protected getPropIntValue(data:any):number|undefined{
    ...
    // 将字符串类型转为数字类型
    protected getPropNumberValue(data:any):number|undefined{
        data = this.invokePropValue(data);
        if(data)
    ...
    // 将值转换为字符串
    protected getPropStringValue(data:any):string|undefined{
        data = this.invokePropValue(data);
        if(data){
            return data + "";
        }else
    ...
    // 获取属性的数组值
    protected getPropStringArrayValue(data:any,split?:string):Array<any>|undefined{
        if(data){
            data = this.invokePropValue(data);
