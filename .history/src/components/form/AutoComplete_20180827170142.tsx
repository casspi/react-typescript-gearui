...
import { UUID, ObjectUtil, Http } from '../../utils';
...
export default class AutoComplete<P extends typeof props & InputProps, S extends state & InputProps> extends Text.default<P, S> {
    ...
    getInputProps() {
        ...
        return G.G$.extend({}, state, {
            onFocus: (e: any) => {
                ...
                if(state.mustMatch == true && !state.searchOptions){
                    // 重新加载数据
                    this.search(value);
                }              
            },
     ...
     * @param optionsParam 
     */
    public getOption(value: string, optionsParam?: OptionData[]): OptionData | null {
            ...
            if(ObjectUtil.isEmpty(this.props.value) == false){
                if(this.state.mustMatch == true){
                    ...
                    if(options && options.length==1){
                        this.setValue();
                        this.triggerChange(options[0].props.value);
                        this.setState({
                            value:options[0].props.value,
                            text:options[0].props.text,
                        });
                    }else{
