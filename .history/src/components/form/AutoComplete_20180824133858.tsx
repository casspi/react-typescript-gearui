...
export default class AutoComplete<P extends typeof props & InputProps, S extends state & InputProps> extends Text.default<P, S> {
    ...
    getAutoCompleteProps() {
        return {
            allowClear : false,
            className : "autocomplete-control " + (this.props.className||""),
            style : { width: this.props.dropdownwidth || this.gwidth ,height:this.gheight},
            ...
            onChange : (value) => {
                if(this._mustMatch==true){
                    ...
                    if(Util.isValueEqual(v,this.getValue())){
                        this.setValue(v,t);
                    }else{
                        this._change(v,t);
                    }
